...
HRESULT image::D_Init(LPCWSTR pFileName, float width, float height, int frameNumX, int frameNumY)
{
	if (pFileName == NULL) return E_FAIL;
	...
	// 첫 번째 프레임을 사용할 수 있는 객체 구성
	hr = _imageInfo->pWICDecoder->GetFrame(_gifFrame, &_imageInfo->pWICFrameDecoder);
	assert(hr == S_OK);
...
void image::gifRender(float destX, float destY, float opacity)
{
	float x_pos = destX - CAMERAMANAGER->get_camera_x();
	float y_pos = destY - CAMERAMANAGER->get_camera_y();

	//흠...
	if (_imageInfo->pWICDecoder->GetFrame(_gifFrame+1, &_imageInfo->pWICFrameDecoder) == NULL)
	{
		//이미지정보 디코더가 i번째 프레임을 가짐.
		_imageInfo->pWICDecoder->GetFrame(_gifFrame, &_imageInfo->pWICFrameDecoder);

		// 포맷 컨버터 생성
		_imageInfo->pWICImagingFactory->CreateFormatConverter(&_imageInfo->pWICFormatConverter);

		// 비트맵으로 변환
		_imageInfo->pWICFormatConverter->Initialize(_imageInfo->pWICFrameDecoder, GUID_WICPixelFormat32bppPBGRA,
			WICBitmapDitherTypeNone, NULL, 0.0f, WICBitmapPaletteTypeCustom);

		// 변환된 이미지 형식을 사용하여 D2D용 비트맵 생성
		D2DMANAGER->pRenderTarget->CreateBitmapFromWicBitmap(_imageInfo->pWICFormatConverter, NULL, &_imageInfo->pBitmap);
		...
		_imageInfo->pWICImagingFactory->Release();
		_imageInfo->pWICFrameDecoder->Release();
		_imageInfo->pWICFormatConverter->Release();
		_imageInfo->pBitmap->Release();
...
void image::FrameRender(float destX, float destY, int currentFrameX, int currentFrameY, float opacity)
{
	...
	if (_imageInfo->pBitmap != NULL)
	{
		D2D1_SIZE_F size = _imageInfo->pBitmap->GetSize();
		...
		D2D1_RECT_F dxArea2 = RectF((currentFrameX * _imageInfo->frameWidth) * ratioX, (currentFrameY * _imageInfo->frameHeight)* ratioY,
			((currentFrameX + 1) * _imageInfo->frameWidth) * ratioX, ((currentFrameY + 1) * _imageInfo->frameHeight) * ratioY);
...
void image::aniRender(float destX, float destY, animation* ani, float opacity)
{
	...
	// 가져올 이미지의 시작점 좌표, 너비, 높이
	D2D1_RECT_F dxArea2 = RectF((ani->getFramePos().x) * ratioX, y * ratioY,
		(ani->getFramePos().x + _imageInfo->frameWidth) * ratioX, (ani->getFramePos().y + _imageInfo->frameHeight) * ratioY);
