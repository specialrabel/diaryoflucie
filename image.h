...
public:
	...
	HRESULT D_Init(LPCWSTR pFileName, float width, float height, int frameNumX, int frameNumY);
	...
	//gif렌더
	void gifRender(float destX, float destY, float opacity = 1.0f);
	...
	inline bool getEndFrame() { return _endFrame; }
	inline void setEndFrame(bool isEnd) { _endFrame = isEnd; }
};
